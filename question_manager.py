...
class QuestionManagerApp:
    def __init__(self):
        ...
        self.question_checkbox_vars = {} # For bulk actions
        self.thumbnail_cache = {} # Performance: Cache for thumbnails
        self.current_page = 1 # Performance: For pagination
        self.display_job = None # Performance: To manage progressive display
    ...
    def setup_top_bar(self):
        # Create a container for the top area
        ...
        # Actions (Right) - Packed Right to Left
        ...
        def worker():
            try:
                ...
                def do_initial_setup():
                    ...
                    # Set defaults to ensure thumbnails can load
                    if self.buckets:
                        self.selected_bucket.set(self.buckets[0])

                    # Set default section and trigger the first refresh
                    if self.sections:
                ...
                # Run messagebox on main thread to avoid Tkinter errors
    ...
    def upload_images(self):
        ...
        # --- التحسين: جلب section_id مرة واحدة فقط ---
        try:
        ...
        for file_path in files:
            try:
                ...
                supabase.table("questions").insert({
                    ...
                    "section_id": section_id, # استخدام الـ ID الذي تم جلبه مسبقًا
                    "answer_type": "mcq",
                    "correct_answer": "A",
    ...
    def display_questions(self):
        # Performance: Cancel any previous rendering job
        if self.display_job:
        ...
        self.question_checkbox_vars.clear() # Clear old checkbox states

        if not self.current_questions:
            ctk.CTkLabel(self.questions_frame, text="No questions found for this section.").pack(pady=20)
        else:
            # Performance: Render questions progressively
            question_iterator = iter(self.current_questions)
            self._display_next_question(question_iterator)

    def _display_next_question(self, iterator):
        try:
            question = next(iterator)
            self._create_question_widget(question)
            self.display_job = self.app.after(15, self._display_next_question, iterator)
        except StopIteration:
            self.display_job = None # Finished

    def _create_question_widget(self, q):
        bucket = self.selected_bucket.get()
        ...
        # Thumbnail
        img_label = ctk.CTkLabel(frame, text="Loading...", width=480, height=480)
        ...
        if PIL_AVAILABLE and bucket and q.get("image_path"):
            ...
            img_label.configure(text="[No Preview]")

        ctk.CTkLabel(frame, text=os.path.basename(q["image_path"])).pack(side="left", padx=10)

        # Correct Answer
        answer_frame = ctk.CTkFrame(frame, fg_color="transparent")
        answer_frame.pack(side="left", padx=10)
        ctk.CTkLabel(answer_frame, text="Answer:").pack(side="left")
        answer_var = ctk.StringVar(value=q.get('correct_answer', 'A'))
        answer_entry = ctk.CTkEntry(answer_frame, textvariable=answer_var, width=50)
        answer_entry.pack(side="left", padx=5)
        answer_entry.bind("<Return>", lambda event, q_id=q['id'], entry=answer_entry: self.update_answer_from_entry(event, q_id, entry))

        # Buttons
        ctk.CTkButton(frame, text="✏ Edit", width=70, command=lambda q=q: self.edit_question(q)).pack(side="right", padx=5)
        ctk.CTkButton(frame, text="🗑 Delete", width=70, fg_color="red", command=lambda q=q: self.delete_question(q)).pack(side="right", padx=5)
        ctk.CTkButton(frame, text="📦 Move", width=70, command=lambda q=q: self.move_question(q)).pack(side="right", padx=5)

    # === EDIT ===
    def edit_question(self, question):
        """Opens a properties dialog for a single question."""
        self.open_properties_dialog([question])

    def update_answer_from_entry(self, event, question_id, entry_widget):
        """Updates the correct answer directly from the entry widget in the list."""
        ...
        try:
            ...
            self.app.after(2000, lambda: entry_widget.configure(border_color=ctk.ThemeManager.theme["CTkEntry"]["border_color"]))

            # Update local data model to avoid full refresh
            for question in self.current_questions:
                if question['id'] == question_id:
                    question['correct_answer'] = new_answer
                    break
        except Exception as e:
            entry_widget.configure(border_color="red") # Visual feedback for error
            self.show_error("Failed to update answer", e)
    ...
    def move_selected_questions(self):
        """Moves all selected questions to a new section."""
        selected_ids = self.get_selected_question_ids()
        if not selected_ids:
            messagebox.showinfo("Info", "No questions selected.")
            return
    ...
    def bulk_edit_selected_questions(self):
        """Opens a dialog to edit properties for all selected questions."""
        selected_ids = self.get_selected_question_ids()
        if not selected_ids:
            messagebox.showinfo("Info", "No questions selected.")
            return

        # Create a list of dummy question objects for the dialog
    ...
    def open_properties_dialog(self, questions_to_edit: list):
        """A generic dialog to edit properties for one or more questions."""
        ...
        update_dialog_view() # Set initial state
        ...
        def on_save():
            ...
            # ✅ تعديل: منطق إضافي لتنظيف الحقول عند تغيير النوع
            new_answer_type = answer_type_var.get()
            if new_answer_type != '(No Change)':
                if new_answer_type == 'numeric':
                    # عند التحويل إلى رقمي، قم بتفريغ الإجابة النصية/الاختيارية
                    payload['correct_answer'] = None
                else:
                    # عند التحويل من رقمي إلى أي نوع آخر، قم بتفريغ الإجابات الرقمية المقبولة
    ...
    def ask_for_section_dialog(self):
        """Shows a custom dialog to select a section from a dropdown."""
        dialog = ctk.CTkToplevel(self.app)
    ...
    def show_full_image(self, bucket, path):
        ...
        top.transient(self.app) # Keep window on top of the main app
        ...
        def load_and_display():
            ...
            try:
                ...
                def update_ui():
                    ...
                    top.image = ctk_img  # Keep a reference
    ...
    def _execute_with_retry(self, action, max_attempts=5, initial_delay=1.0):
        """Executes a function with an exponential backoff retry mechanism for network operations."""
        def is_retryable(e):
            err_str = str(e).lower()
            # Catches 404s, and 400s that are actually 404s inside the message body
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if is_retryable(e) and attempt < max_attempts - 1:
                    delay = initial_delay * (2 ** attempt) # 1, 2, 4, 8 seconds
    ...
    def show_error(self, user_message, error_exception):
        """Security: Logs detailed error and shows a generic message to the user."""
        logging.error(f"{user_message}: {error_exception}")
        messagebox.showerror("Error", f"{user_message}.\nSee logs for more details.")

    # === RUN ===
    def run(self):
        # The initial refresh is now triggered by load_sections after data is ready.
        self.app.mainloop()
